...
- **效果**：提升药水投掷速度和弹道平直度
- **附魔台最高等级**：10级（仅可附到书上，无法直接附到药水/物品上）
- **铁砧合法最高等级**：19级（无法触发>20级射线追踪模式，保证超级金苹果迅投25的稀缺性）
...
### 9.1 调试菜单与 HUD（独立 Mod）

调试菜单、实体血量 HUD、手持物品 HUD、玩家行为日志等辅助功能已拆分为独立的 **debug-menu** Mod。

- **不安装 debug-menu**：主 Mod 正常运行，无调试 UI 和 HUD 功能
- **安装 debug-menu**：主 Mod 自动注册 9 个调试日志开关，调试菜单中统一显示
- **第三方 Mod 接入**：任何 Mod 可通过 `DebugMenuApi.register()` 注册自己的调试开关

debug-menu Mod 提供的功能：
- 实体血量 HUD（准星瞄准实体时显示血量，支持详细 NBT 信息）
- 手持物品 HUD（左上角显示主手物品，支持高级 NBT 模式）
- 调试日志开关菜单（动态读取所有注册的开关，按 Mod 分组显示）
- 玩家行为实时日志（攻击/交互/受伤/死亡/移动等）
- 通过自定义按键打开（默认无绑定）

### 9.2 配置持久化

- 调试日志开关配置文件位于 `config/better-enchanted-debug.json`
- 所有设置变更自动保存，启动时自动加载
- 格式为可读的 JSON，支持手动编辑
...
| 迅投 (Swift Throw) | — | ✅ | ∞（合法最高19） | 自定义附魔，附魔台最高10级（仅书），铁砧合法最高19级 |
