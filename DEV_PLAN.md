...
- ✅ **迅投 (Swift Throw)** — 自定义药水附魔，提升投掷物初速度，每级+50%（公式：实际初速度 = 原始初速度 × (1 + 0.5 × 等级)），附魔台最高等级10（仅可附到书上），铁砧合法最高等级19；等级>20时进入射线追踪瞬移模式（隐藏药水实体+暴击粒子弹道+精确对准准星），发射角度公式 y=80/(4+x)，等级越高越平射
