...
- ✅ **无限 (Infinity) [药水]** — 掷出的药水不消耗；自定义冷却系统（基于NBT标记InfinityMarked），只影响带无限附魔的物品不影响同种普通物品；若有耐久则判定：成功不进冷却，失败进入30s冷却；无耐久则始终30s冷却；客户端通过网络同步显示灰色半透明冷却动画覆盖
- ✅ **引雷 (Channeling) [药水]** — 雷暴天气时，药水落点4格范围内所有实体（需露天）各召唤一道闪电，基于MC 1.20.4三叉戟引雷规则改良为AOE版本
- ✅ **多重射击 (Multishot) [药水]** — 多等级圆锥散布系统：总投掷物=2+level（Lv I=3, Lv II=4...），第1个直飞，额外投掷物前8个均匀分布在10°圆锥圆周上，超过8个向圆内随机投掷，兼容迅投附魔所有模式
- ✅ **快速装填 (Quick Charge) [药水]** — 减少无限附魔的冷却时间，每级减少20%冷却时间（Lv I=24s, Lv II=18s, Lv III=12s, Lv IV=6s, Lv V=0s）
- ✅ **穿透 (Piercing) [药水]** — 参考MC 1.20.4弩穿透规则：药水可穿过level个实体（总共命中level+1个），穿透时对每个实体执行附魔效果（锋利/力量/冲击/火矢），已穿透实体不重复命中，命中方块或达到穿透上限时正常销毁并触发溅射效果
- ✅ **忠诚 (Loyalty) [药水]** — 参考MC 1.20.4三叉戟忠诚规则改编：药水掷出2s后自动返回投掷者，返回速度=0.5×level格/tick（Lv I=10格/s, Lv II=20格/s, Lv III=30格/s），掷出/返回途中碰到实体或方块则正常消耗，成功返回则归还物品给玩家
...
| 无限 (Infinity) | 掷出的药水不消耗；若有耐久则判定：成功不进冷却，失败进入30s冷却 | ✅ 已完成 |
...
| 附魔 | 效果 | 难度 |
|------|------|------|
| 多重射击 (Multishot) | 多等级圆锥散布：总投掷物=2+level，前8个额外投掷物均匀分布10°圆周，超出部分圆内随机，只消耗1个 | ✅ 已完成 |
| 快速装填 (Quick Charge) | 减少无限附魔的冷却时间，每级减少20%冷却时间（Lv I=24s, Lv II=18s, Lv III=12s, Lv IV=6s, Lv V=0s） | ✅ 已完成 |
| 穿透 (Piercing) | 掷出的药水穿透实体，对穿过的实体进行击中判定 | ✅ 已完成 |

### 四、药水类附魔 — 三叉戟（对掷出的药水生效）

| 附魔 | 效果 | 难度 |
|------|------|------|
| 忠诚 (Loyalty) | 药水掷出2s后自动返回，掷出/返回时碰到实体或方块则消耗掉 | ✅ 已完成 |
...
15. **快速装填 (Quick Charge)** — 减少无限附魔的冷却时间  
    - 实现方式：在无限附魔的冷却计算逻辑中检测是否带有快速装填
    - 每级减少20%冷却时间（基础30s × (1 - 0.2 × level)）
    - Lv I = 24s, Lv II = 18s, Lv III = 12s, Lv IV = 6s, Lv V = 0s（无冷却）
    - 需要修改现有无限附魔的冷却设置代码，读取 Quick Charge 等级

16. **穿透 (Piercing)** — 药水穿透实体  
    - 修改药水实体的碰撞逻辑，命中后不消失
    - 对穿过的每个实体进行一次击中判定
    - 需要记录已击中的实体避免重复判定
...
17. **忠诚 (Loyalty)** — 药水投掷2s后自动返回  
    - 最复杂的效果之一
    - 药水实体需要自定义 tick 逻辑：2s后改变速度方向飞回玩家
    - 返回途中碰到实体或方块则消耗掉
...
- 抢夺 (Looting)
- 横扫之刃 (Sweeping Edge)
- ~~快速装填 (Quick Charge)~~ — 已规划（减少无限冷却时间）
- 激流 (Riptide)
- 穿刺 (Impaling)
